...
This case study includes a little bit more complexity than the first one (bat diets) and covers a few new topics in R and in general. The complexity mainly occurs in the calculations we do to obtain the response variables (i.e. the variables we examine to answer our biological question). The calculations involve some conceptually challenging manipulations, and the corresponding pipelines of **dplyr** functions will at first seem complex and daunting.
...
Again: as you work through this chapter, try following along with the [workflow/checklist here](#insights-workflow). 
...
```{r insight-sketch-4, fig.cap='A hypothetical outcome of the demonstration study of how a manipulation of diet composition and amount affect the lifespan of fish.', out.width='70%', fig.asp=.85, fig.align='center', echo=FALSE}
knitr::include_graphics("images/insights_sketches_004.jpeg")
```
...
Good, now `count` is a numeric type variable.
...
### Make the `prey_composition` variable a factor with specific order {#factors-predstab}
...
The dynamics in Figure \@ref(fig:microbe1) are those shown in figure 1 of the original publication. Everything seems reasonable, though I (Owen) wish I'd recorded more information about why I stopped sampling the microcosms when I did.
...
```{r microbe2, fig.cap='Histograms of the three response variables.', out.width='70%', fig.align='center', fig.height=2}
rv %>%
  ggplot() +
...
```{r microbe3, echo=FALSE, fig.cap='Histogram of log10 maximum abundance. We thought that log transformation would make the distribution more symmetric, but it has not... it just made it asymmetric in the opposite direction!', out.width='50%', fig.align='center'}
rv %>%
...
```{r microbe4-1, echo=TRUE, fig.cap='A reasonably nice graph of maximum predator density in a community against the number of prey available to the predator.', out.width='100%', fig.align='center', fig.width=8}
rv %>%
...
```{r microbe5-1, echo=TRUE, fig.cap='A quite informative graph of predator population variability (measured as the coefficient of variation--CV) in a community against the number of prey available to the predator.', out.width='100%', fig.align='center', fig.width=8}
rv_oi <- "CV"
rv %>% filter(Response_type == rv_oi) %>%
  ggplot() +
  geom_jitter(aes(x = prey_richness, y = Value, col = prey_composition),
...
1. Use your best judgment about if a value seems extreme. If you're in doubt, consider it extreme. Nothing we subsequently do with these apparently extreme values will cause problems if we consider some values as being extreme that other people might not.

2. Check that each extreme value is not the result of an error. E.g. in data entry, or subsequent assumptions in calculations. If you ever meet us in person, ask us to tell you the badger outlier story.
...
4. Have a look at the [influence and outliers section](#influence-outliers) of this web site.
