...
stab %>%
  ggplot() +
  geom_point(aes(x = day, y = log10(dens_per_ml), col = species)) + 
  geom_line() +
...
## ----microbe4-1, echo=FALSE, fig.cap='A reasonably nice graph of maximum predator density in a community against the number of prey available to the predator.', out.width='100%', fig.align='center', fig.width=8----
...
## ----microbe5-1, echo=FALSE, fig.cap='A quite informative graph of predator population variability (measured as the coefficient of variation--CV) in a community against the number of prey available to the predator.', out.width='100%', fig.align='center', fig.width=8----
rv_oi <- "CV"
rv %>% filter(Response_type == rv_oi) %>%
  ggplot() +
  geom_jitter(aes(x = prey_richness, y = Value, col = prey_composition),
...
pred_CV %>%
  ggplot() +
  geom_jitter(aes(x = prey_composition, y = CV),
              width = 0.1, height = 0) +
  coord_flip() +
