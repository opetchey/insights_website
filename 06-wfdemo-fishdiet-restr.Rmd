...
In this final Workflow Demonstration we, your instructors take a step back, and invite you to be more independent. There will be less guidance, less instruction, and more challenges for you. In a sense then, this is less of a Workflow Demonstration, and more of a Workflow Challenge.

For this Workflow Demonstration we turn to published data from a study of the effects of dietary restriction (DR). For some general background on dietary restriction, please look here https://www.nia.nih.gov/health/calorie-restriction-and-fasting-diets-what-do-we-know
...
**Important**: There are two available versions of some of the datasets for this study. Some of the original (non-updated) versions contained some very minor errors that the researchers then corrected. In this Workflow Demonstration we first challenge you to find those errors in the original datasets, and then to check the updated datasets do not contain them errors, and to then continue with those updated datasets.
...
**Individual characteristics we will here focus on are:**
...
**Type of response variable for each individual characteristic:**
...
In the mortality dataset fish LR504 has two record for week 2.

```{r}
check <- mortality %>% group_by(Fish_ID) %>%
