---
title: "Case Study 3 - global food diversity"
output: html_document
---

# Case study -- Are diets more diverse in more democratic countries?  {#CS3-1}
...
```{r}
filter(fbs_long, Country == "Albania", Item == "Eggs", Year == 1961)
```
...
```{r}
dd <- left_join(fao_div, pol)
sum(is.na(pull(dd, Polity2)))
```
...
We have a lot of objects in R's brain. We only need one: `dd`. We can remove all the other variables. We don't need to run all the code above every time we do an analysis. In this situation we can save the `dd` dataset, remove everything from R's brain using `rm(list = ls())` or by clicking the little broom icon in RStudio, and load it again:

```{r eval=FALSE}
saveRDS(dd, "data/fao_pol.Rdata")
```

```{r}
rm(list = ls())
dd <- readRDS("data/fao_pol.Rdata")
```
...
ggplot(dd, aes(x = richness, y = diversity)) +
...
ggplot(dd, aes(x = Polity2, y = richness)) +
  geom_point(alpha = 0.05)
```
...
ggplot(dd, aes(x = Polity2, y = diversity)) +
  geom_point(alpha = 0.05)
```
