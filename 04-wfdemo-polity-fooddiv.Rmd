**Rename datasets from dd_??? to something informative. (search for dd!)**
...
This Workflow Demonstation (WD) is primarily meant to supplement the material in the book *Insights from Data with R* by Petchey, Beckerman, Childs, and Cooper. If you don't understand something here, have a look at that book (perhaps again), or search for help, or get in touch with us.
...
### Variables describing interventions/manipulations


### Amount of correlation among variables


### How independent are variables
...
(Please note that here and below we do not describe or "teach" the R code used. This is because it is described in detail in the book.)
...
If you do want to go through the process of getting the 206MB file, and reducing it down to something more manageable, feel free to use the code immediately below, otherwise skip over this. It's pretty straightforward, apart from two things.
...
Feel free to have a look at the dataset with the `glimps` function. We `select` a few columns first, as otherwise the output is rather long due to there being 112 variables in the dataset.
...
If the data contained every possible combination of country and item we would expect 182 * 114 = 20'748 observations. That there are only 17'953 tells us that some combinations are missing. We will find out which these are.

The six year variables are, as expected numeric---they are amounts of food available (food supply quantities in kg per capita per year).
...
There are, in fact, 9,287 rows that are duplicated (not unique). Let's assume (somewhat dangerously perhaps) that these are mistaken duplicate entries, and so reduce to a dataset with only unique rows:
...
Looking at the result we see there are 10,733 rows with duplicated Country, Year, and Item. This is weird. And these these duplicates are only for food items "Eggs" and "Milk - Excluding Butter". We have no idea why there are multiple entries for these two items! Here's an example of one of the duplicates:
...
The final step of data preparation is to merge the Polity and FAO data. One nasty issue we must deal with before we do the merge is to check the country names in the two datasets. If both datasets had been made with **interoperability** in mind, they would have used a standard naming of countries, such as ISO (International Organisation for Standardization) 3166-1. We acquired this standard from https://datahub.io/core/country-list. Here we read it into R:

```{r}
ISO_country_names <- read_csv("data/ISO_country_data_csv.csv") %>%
  rename(ISO2_Code = Code)
```

```{r}
fbs_countries <- unique(pull(fbs_div, Country))
iso_countries <- pull(ISO_country_names, Name)
setdiff(fbs_countries, iso_countries)
setdiff(iso_countries, fbs_countries)

```


Seems odd that FAO would not have used standardised (ISO 3166-1) naming. Digging into their metadata a bit we can find a page of Definitions and Standards: http://www.fao.org/faostat/en/#definitions. In the tab *Country/Region* we find a dataset with country names and various country codes, including ISO2 and ISO3 codes. Excellent. We downloaded this dataset and now read it into R and merge with the food balance sheet dataset `fbs`:

```{r}
FAO_country_standards <- read_csv("data/FAOSTAT_data_country_name_standards.csv")
...
Swaziland in Food Balance sheet is ...Eswatini!!!

```{r}
fbs_country_name_code <- read_csv("data/FoodBalanceSheets_E_All_Data.csv",
               locale = locale(encoding = 'ISO-8859-1'))
fbs_country_name_code <- fbs_country_name_code %>%
...
fbs_div_standardised <- full_join(fbs_div, fbs_country_name_code) %>%
  rename(Country_name_fbs = Country) %>%
  full_join(FAO_country_standards) %>%
  select(-Start_Year, -End_Year)

merge_check <- fbs_div_standardised %>%
  filter(is.na(Country_name_fbs) |
         is.na(Year) |
         is.na(diversity) |
         is.na(richness) |
         is.na(Country_Code) |
         is.na(Country_name_FAO) |
         is.na(M49_Code) |
         is.na(ISO2_Code) |
         is.na(ISO3_Code)) %>%
  select(-Year, -diversity, -richness) %>%
  unique()
                    

#%>%
#  full_join(ISO_country_names)
```


First let us deal with making the FAO data interoperable. The dataset (as downloaded from the FAO) does not contain ISO country names/codes


It's quite possible, even likely, there are instances of the same country having different names in the two. The following code gets the country names from each, looks for ones that appear in one and not the other, and vice versa, and writes these to csv files.

```{r eval=FALSE}
pol_countries <- unique(pull(pol, Country))

in_pol <- setdiff(pol_countries, fbs_countries)
in_fbs <- setdiff(fbs_countries, pol_countries)
write_csv(tibble(in_pol), "data/in_pol.csv")
write_csv(tibble(in_fbs), "data/in_fbs.csv")
```
...
pol1 <- left_join(pol, countryname_map, by =c ("Country" = "in_pol")) %>%
  mutate(Country = ifelse(is.na(in_fbs),
                                Country,
                                in_fbs)) %>%
  select(-in_fbs)
#skim(pol1)
...
```{r}
fbs_pol <- left_join(fbs_div, pol)
sum(is.na(pull(fbs_pol, Polity2)))
```

We get 2,607 missing polity values. And here after the renaming:

```{r}
dd1 <- left_join(fbs_div, pol1)
...
As usual we look at the distribution of our variables. First the distribution of number of food items available in each country, across all countries and all years. The code for this you are quite familiar with by now, using `ggplot` and `geom_histogram` (Figure \@ref(fig:fbs-pol2):
...
ggplot(fbs_pol, aes(x = diversity)) +
  geom_histogram(bins = 10)
```
...
ggplot(fbs_pol, aes(x = richness, y = diversity)) +
...
ggplot(fbs_pol, aes(x = Polity2, y = richness)) +
  geom_point(alpha = 0.05)
```
...
ggplot(fbs_pol, aes(x = Polity2, y = diversity)) +
  geom_point(alpha = 0.05)
```
