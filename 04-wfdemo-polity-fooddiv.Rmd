...
First let's first clear R and reload the two datasets:
...
What a mess!!! The FAO data uses "United States of America" while the polity data uses "United States". And notice the different names for Tanzania. We need a standard.

Both original datasets contain country *codes* as well as names. First look at the FAO food balance raw data. It contains the variable "Country Code":
...
And put the code into the `fbs_div` dataset. There both contain the common `Country` variable we want to merge by:
...
```{r}
FAO_country_standards %>% filter(str_detect(Country_name_FAO, "United"))
```
```{r}
FAO_country_standards %>% filter(str_detect(Country_name_FAO, "Tanz"))
```
```{r}
...
```{r}
pol %>% filter(str_detect(Country, "Swaz")) %>%
  pull(Country) %>%
  unique()
```
...
```{r}
pol_iso_mapping <- read_csv("data/pol_iso_mapping.csv", na=".") %>%
...
Use inner join to get keep only the rows with entries for both food diversity and polity. There can be a maximum number of rows of the smaller dataset, here 8'272 for the food diversity dataset.
