...
The table in Figure \@ref(fig:classic-dplyr-comparison) below compares common data manipulation methods using the classic way (base R) and the modern way, with functions in the dplyr package. The placement of the comma, when using `[ ]`, is quite important and subtle. The help files for `subset()`, `order()`, `aggregate()`, and `tapply()` are worth consulting should you run into these functions.


```{r classic-dplyr-comparison, fig.cap='Comparison of classic/base R methods and dplyr methods for common operations.', out.width='70%', fig.asp=.75, fig.align='center', echo=FALSE}
knitr::include_graphics("images/classic_dplyr_comparison.png")
```
