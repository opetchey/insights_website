...
All of the biological questions we so far addressed were univariate, in the sense that analysis of only one response variable was enough. Many kinds of biological question involve multiple response variables, however, and these are multivariate problems. They require multivariate data, methods for visualizing multivariate data, and methods for analyzing it. This chapter covers the three issues.

Consider the question of how human skull shape has changed during the last few millennia. If we measured any single dimension of a skull, for example, maximum breadth, this question about shape would be beyond us. We might find that maximum breadth had increased, but this could be associated with increases in every measurement. Such a finding would indicate that skulls had increased in size, but not changed in shape. But with only one measurement, we could not know.
